...
class ST_Container_Close_Block extends AQ_Block {
	...
	?>
		<!--<p class="description third">
			<label for="<?php echo $this->get_field_id('taghtml') ?>">
				add tag: <code> div, aside, article </code>
				<?php echo aq_field_input('taghtml', $block_id, $taghtml, $size = 'full') ?>
			</label>
		</p>
		
		<p class="description third">
			<label for="<?php echo $this->get_field_id('closetag') ?>">
				default tag is outside the container<br>
				<?php echo aq_field_checkbox('closetag', $block_id, $closetag) ?>
